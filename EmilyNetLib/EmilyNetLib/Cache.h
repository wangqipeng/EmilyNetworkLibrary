...
int Cache<Key,Value>::add(const Key& k, const Value& v)
{
    ...
    if(iter != hash_map_.end())
    {
        FILE_LOG(logERROR)<<"ERR in add the Key-Value to the Cache, the Key has existed\n";
        return -1; 
    }
...
int Cache<Key,Value>::replace(const Key& k, const Value& v)
{
    ...
    if(iter == hash_map_.end())
    {
        FILE_LOG(logERROR)<<"ERR in replace the Key-Value in the Cache,the Key doesn't exist";
        return -1;   
    }   
