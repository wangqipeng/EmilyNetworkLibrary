    ...
    if(0 == readable)
    {
        ...
        if(nwrote >= 0)
        {
            if(nwrote <= len)
            {
                ...
                FILE_LOG(logINFO)<<__FILE__<<" "<<__FUNCTION__<<" "<<__LINE__<<" write "<<nwrote<<" bytes";
            }
                //write complete
        ...
        else
        {
            ...
            { 
                ...
                FILE_LOG(logERROR)<<__FILE__<<" "<<__FUNCTION__<<" "<<__LINE__<<" "<<strerror(errno);
            }
        }
    }
...
void Connection::HandleReadEvent()
{
    ...
    FILE_LOG(logINFO)<<__FILE__<<" "<<__LINE__<<__FUNCTION__<<" Connection::HandleReadEvent";		
    ...
    if(total > 0)
    {
        ...
        FILE_LOG(logINFO)<<__FILE__<<" "<<__LINE__<<__FUNCTION__<<" run message callback ";	
    ...
    {
        FILE_LOG(logERROR)<<__FILE__<<" "<<__LINE__<<__FUNCTION__<<" error: "<<strerror(errno);		
    }
}
...
void Connection::HandleWriteEvent() 
{
    ...
    if(n > 0)
    {
        ...
        {
		    FILE_LOG(logINFO)<<__FILE__<<" "<<__LINE__<<" write more data";
        }
    }
    else
    {
        FILE_LOG(logERROR)<<__FILE__<<" "<<__LINE__<<" Connection::HandleWriteEvent error";
    }
}
...
void Connection::HandleErrorEvent()
{
    ...
    FILE_LOG(logERROR)<<__FILE__<<" "<<__FUNCTION__<<" "<<strerror(err);
}

void Connection::BindEventCallback()
{
    FILE_LOG(logINFO)<<__FILE__<<" "<<__FUNCTION__<<" "<<__LINE__;
     //handle_.BindConnectionCallback(conn_callback_);
    handle_->BindReadCallback(boost::bind(&Connection::HandleReadEvent, this));
