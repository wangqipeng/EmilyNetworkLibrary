...
int TimerMonitor::CreateTimerfd()
{
    ...
    FILE_LOG(logINFO)<<"TimerMonitor::CreateTimerfd "<<__LINE__<<" fd:"<<fd<<" successful";
    return fd;
}
...
{
	FILE_LOG(logINFO)<<__FILE__<<" "<<__FUNCTION__<<" "<<__LINE__<<" inactive timers: "<<InactiveTimerCount();
...
void TimerMonitor::ResetTimerfd()
{
    ...
    if(ActiveTimerCount() > 0)
    {
		...
		FILE_LOG(logINFO)<<__FILE__<<" "<<__FUNCTION__<<" "<<__LINE__<<" timeout: "<<timeout;
...
void TimerMonitor::ProcessExpiredTimers()
{
    ...
    size_t i = 0; 
	FILE_LOG(logINFO)<<__FILE__<<" "<<__LINE__<<"total timers :"<<total_timers;
    while(i < total_timers)
	{
        Timer* top = FirstExpired(); 
	    Time now(Time::NowTimeUs());
	    Time expired(top->GetTime());
	    FILE_LOG(logINFO)<<__FILE__<<" "<<__LINE__<<" now: "<<now.GetFormatTime()<<" expired: "<<expired.GetFormatTime();
	    if(expired < now)
        {
	        FILE_LOG(logINFO)<<__FILE__<<" "<<__LINE__<<" callback run at ";//FIXME:use format time
...
void TimerMonitor::ReadTimerfd()
{
	...
	if(n != sizeof(int64_t))
	{
		FILE_LOG(logERROR)<<__FILE__<<" "<<__LINE__<<" read timerfd error"<<strerror(errno);
	}
}
